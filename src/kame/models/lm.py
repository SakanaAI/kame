...
class LMModel(StreamingContainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.oracle_emb = EmbeddingFactory(text_card + 1, dim, demux_second_stream=demux_second_text_stream)
    ...
    def forward(
        ...
        # oracle tokens handling
        if oracle_tokens is not None:
            # oracle_tokens is [B, 1, T]
            if oracle_tokens.shape != (B, 1, T):
                raise ValueError(
                    f"Invalid oracle_tokens shape: expected (B, 1, T) which is ({B}, 1, {T}), "
                    f"but got {tuple(oracle_tokens.shape)}."
                )
            oracle_initial = self._get_initial_token()[:, :1].expand(B, -1, -1)
            oracle_tokens = torch.cat([oracle_initial, oracle_tokens], dim=2)
        ...
        transformer_out, text_logits = self.forward_text(
            ...
            sum_condition,
            cross_attention_src,
            oracle_tokens=oracle_tokens[:, :, :-1] if oracle_tokens is not None else None,
        )
    ...
    def forward_text(
        ...
        # oracle tokens addition
        if oracle_tokens is not None:
            # oracle_tokens: [B, 1, S] -> [B, S]
            if oracle_tokens.dim() == 3:
                oracle_tokens = oracle_tokens[:, 0]
            oracle_emb = self.oracle_emb(oracle_tokens)
...
class LMGen(StreamingModule[_LMGenState]):
    def __init__(
        ...
        # added for oracle implementation.
        self.oracle_tokens: torch.Tensor | None = None

        # Stores the offset_cpu value at the time oracle_tokens was set.
        # This allows computing the relative step index into oracle_tokens:
        #   rel_step = state.offset_cpu - self._oracle_injected_at_offset_cpu
        # so that oracle_tokens[rel_step] is used at each streaming step.
        # Set externally when oracle_tokens is updated (e.g., via monkey-patching).
        self._oracle_injected_at_offset_cpu: int | None = None
    ...
    def _init_streaming_state(self, batch_size: int) -> _LMGenState:
        ...
        # 2 define function for wrapping forward_text
        # Wrapper to include oracle_tokens in the graphed function signature.
        # This allows passing oracle tokens through CUDAGraphed during inference.
        def forward_text_wrapper(input_, sum_cond, cross_cond, oracle_tokens):
            return lm_model.forward_text(
                input_, sum_condition=sum_cond, cross_attention_src=cross_cond, oracle_tokens=oracle_tokens
            )

        disable = lm_model.device.type != "cuda"
        graphed_main = CUDAGraphed(forward_text_wrapper, disable=disable)
    ...
    def _step(
        ...
        if self.oracle_tokens is not None and self._oracle_injected_at_offset_cpu is not None:
            rel_step = state.offset_cpu - self._oracle_injected_at_offset_cpu
            if 0 <= rel_step < self.oracle_tokens.shape[1]:
                token = self.oracle_tokens[:, rel_step].view(1, 1, 1)
                oracle_tokens_step = token.expand(B, 1, 1)

        # Match the doubled CFG batch size
        if self.cfg_coef != 1.0:
            oracle_tokens_step = oracle_tokens_step.repeat(2, 1, 1)

        transformer_out, text_logits = state.graphed_main(
            input_, state.condition_sum, state.condition_cross, oracle_tokens_step
        )
    ...
    def update_oracle_tokens_streaming(self, token_ids: list[int] | None = None, reset: bool = False):
        """Update oracle tokens during streaming inference.

        Args:
            token_ids: Pre-tokenized token IDs. Tokenization should be done externally.
                        If None or empty, no tokens are appended.
            reset: If True, reset oracle tokens to initial state.

        Note:
            This method is NOT thread-safe. It must be called from the same thread/task
            that executes step() to avoid race conditions. In the current server architecture,
            both are called sequentially within opus_loop, so no locking is required.
        """
        if reset or self.oracle_tokens is None:
            start_id = int(self.lm_model.text_initial_token_id)
            self.oracle_tokens = torch.tensor([[start_id]], device=self.lm_model.device)
            if self._streaming_state is not None:
                self._oracle_injected_at_offset_cpu = self._streaming_state.offset_cpu
            else:
                self._oracle_injected_at_offset_cpu = 0

        # Empty list is intentionally ignored (no tokens to append)
        if token_ids:
            new_tokens_tensor = torch.tensor([token_ids], device=self.lm_model.device)
            self.oracle_tokens = torch.cat([self.oracle_tokens, new_tokens_tensor], dim=1)
