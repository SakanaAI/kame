...
def hf_get(filename: str | Path, hf_repo: str | None = None, check_local_file_exists: bool = False) -> Path:
...
def _upgrade_legacy_lm_state_dict(state: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    """Normalize older LM checkpoints to the current LMModel parameter layout."""
    upgraded_state = dict(state)
    if any(key.startswith("oracle_emb.") for key in upgraded_state):
        return upgraded_state

    text_prefix = "text_emb."
    for key, value in state.items():
        if key.startswith(text_prefix):
            oracle_key = f"oracle_emb.{key[len(text_prefix) :]}"
            upgraded_state[oracle_key] = value.detach().clone()
    return upgraded_state
...
def get_moshi_lm(
    ...
    lm_kwargs_overrides: tp.Optional[dict[str, tp.Any]] = None,
) -> LMModel:
    if lm_kwargs is None:
        lm_kwargs = _lm_kwargs
    lm_kwargs = dict(lm_kwargs)
    lm_kwargs_overrides = {} if lm_kwargs_overrides is None else dict(lm_kwargs_overrides)
