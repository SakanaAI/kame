...
def _require_initialized_asr(enable_asr: bool, asr_processor: Optional[OpenAIRealtimeASRProcessor]) -> None:
    if not enable_asr:
        return
    ...
    raise RuntimeError(
        "ASR is enabled but OpenAI Realtime transcription could not be initialized. "
        f"{reason} "
...
class ServerState:
    ...
    lm_gen: LMGen
    lock: asyncio.Lock
    asr_processor: Optional[OpenAIRealtimeASRProcessor] = None
    ...
    def __init__(
        ...
        asr_language: str = DEFAULT_OPENAI_ASR_LANGUAGE,
        **kwargs,
    ):
...
def main():
    ...
    parser.add_argument(
        ...
        help=(f"OpenAI Realtime transcription model to use for ASR (default: {DEFAULT_OPENAI_ASR_MODEL})."),
    )
    parser.add_argument(
        "--asr-language",
        type=str,
        default=DEFAULT_OPENAI_ASR_LANGUAGE,
        help=(f"ISO-639-1 ASR language code for OpenAI transcription (default: {DEFAULT_OPENAI_ASR_LANGUAGE})."),
    )
    parser.add_argument(
        "--ssl",
        type=str,
    ...
    if args.enable_asr:
        log(
            "info",
            (
                f"OpenAI Realtime ASR enabled (model: {args.asr_model}, "
                f"language: {args.asr_language}) - partials stream to LLM; "
